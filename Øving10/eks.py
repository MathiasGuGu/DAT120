...
class SpørreOppgave:
    ...
    def sjekk_svar(self):
        ...
        for i in range(len(spillere)):
            ans = input(f"{spillere[i]}): Velg det alternativet du tror er korrekt: ")
            if ans == self.korrekt_svar:
                spiller_poengsum[i] += 1
            

        print(spiller_poengsum)
